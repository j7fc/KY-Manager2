    ...
    async execute(interaction) {
        ...
        db.get(
            ...
            (err, row) => {
                ...
                if (!row) {
                    return interaction.reply('❌ هذا العضو لا يملك نقاطاً مسجلة.');
                }
                ...
                if (department === 'admin') {
                    ...
                    newPoints = Math.max(0, row.adminPoints - points);
                    ...
                    newPoints = Math.max(0, row.monitorPoints - points);
                ...
                const embed = new EmbedBuilder()
                    .setTitle('❌ تم خصم نقاط')
                    .addFields(
                        { name: '👤 العضو', value: `<@${user.id}>`, inline: false },
                        { name: '📂 القسم', value: department === 'admin' ? 'الإدارة' : 'الرقابة', inline: true },
                        { name: '➖ النقاط المخصومة', value: points.toString(), inline: true },
                        { name: '📝 السبب', value: reason, inline: false },
                        { name: '📊 المجموع الحالي', value: newPoints.toString(), inline: true },
                        { name: '👮 تمت بواسطة', value: interaction.user.username, inline: true }
                    )
                    .setTimestamp();
